...
public class HeroControllerTest {
	...
	public void testControllerHeroA() throws Exception {
		...
		String strJson;
		strJson = mockMvc
				.perform(MockMvcRequestBuilders.post("/stats").contentType(MediaType.APPLICATION_JSON)
						.accept(MediaType.APPLICATION_JSON).content(json))
				.andExpect(status().isOk()).andExpect(jsonPath("$.strength", is(6)))
				.andExpect(jsonPath("$.agility", is(4))).andExpect(jsonPath("$.dexterity", is(3)))
				.andExpect(jsonPath("$.intelligence", is(4))).andReturn().getResponse().getContentAsString();
	...
	public void testControllerHeroB() throws Exception {
		...
		String strJson = mockMvc
				.perform(MockMvcRequestBuilders.post("/hero").contentType(MediaType.APPLICATION_JSON)
						.accept(MediaType.APPLICATION_JSON).content(json))
				.andExpect(status().isOk()).andExpect(jsonPath("$.power_stats.strength", is(6)))
				...
				.andExpect(jsonPath("$.race", is(Race.HUMAN.name()))).andExpect(jsonPath("$.name", is("Volverine")))
	...
	public void testControllerHeroC() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.put("/hero").contentType(MediaType.APPLICATION_JSON)
	...
	public void testControllerHeroD() throws Exception {
		...
		Hero hero = new Hero();
		hero.setId(id);
		mockMvc.perform(MockMvcRequestBuilders.get("/hero/" + hero.getId()).contentType(MediaType.APPLICATION_JSON)
	...
	public void testControllerHeroG() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get("/hero").contentType(MediaType.APPLICATION_JSON)
	...
	public void testControllerHeroH() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders
				.get("/hero".concat("/").concat(hero1.getId().toString()).concat("/").concat(hero.getId().toString()))
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(jsonPath("$.name", is("Volverine Logan")));
	}
	...
	public void testControllerHeroI() throws Exception {
		String json = StrJson.getJson(hero1);
		mockMvc.perform(MockMvcRequestBuilders.delete("/hero").contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON).content(json)).andExpect(status().isOk());
	}
	...
	private Hero getHero() {
		...
		stats.setCreated_at(new Date(2020, 03, 15));
		stats.setUpdated_at(new Date(2020, 03, 16));
