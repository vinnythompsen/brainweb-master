...
public class PowerStatsControllerTest {
	...
	public void testControllerPowerStats() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/stats").contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON).content(json)).andExpect(status().isOk())
				.andExpect(jsonPath("$.strength", is(6))).andExpect(jsonPath("$.agility", is(4)))
				.andExpect(jsonPath("$.dexterity", is(3))).andExpect(jsonPath("$.intelligence", is(4)));
	}
}
